...
trait GetPrerenderedHund
{
    public function getPrerenderedHund($id)
    {

        $prerenderedHund = HundPrerendered::where('hund_id', $id)->first();

        if ($prerenderedHund) {

            return $prerenderedHund;
            ...
            // ZUCHTZULASSUNG
            $str_zuchtzulassung = '';
            if ($hund->zuchtzulassung) {
                $str_zuchtzulassung = 'Ja, ' . $hund->zuchtzulassung->beginn;
            }

            // TOD
            $str_tod = '';
            $str_sterbedatum = '';
            if ($hund->verstorben_id != 0) {
                $str_sterbedatum = $hund->verstorben_datum ? $hund->verstorben_datum : 'unbekannt';
                $str_tod = 'verstorben, Todesursache: ' . $hund->verstorben->todesursache;
            }
            ...
            $hund_prerendered->sterbedatum = $str_sterbedatum;
            $hund_prerendered->verstorben = $str_tod;

            $hund_prerendered->zuchtzulassung = $str_zuchtzulassung;

            $hund_prerendered->zuchtausschlussfehler = $str_zuchtausschlussfehler;

            $hund_prerendered->eigentuemer = $str_personen;
